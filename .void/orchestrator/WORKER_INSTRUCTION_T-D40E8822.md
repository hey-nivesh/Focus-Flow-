...
Ticket ID: T-D40E8822
Title: Search + filter shows no results (Focus-Flow-)
Component: General Support

Description:
User on the local app: after searching and applying a filter, the list is empty even when there should be matches.
Previously user was able to see the recommendations while typing the search bar but now its not working as same.
